...
namespace DB{
    ...
    void insertRequest(std::string ip){
        try{
            ...
            auto result = worker.exec0(R"SQL(CREATE TABLE IF NOT EXISTS t_flow (id INTEGER PRIMARY KEY AUTO INCREMENT, request_time TIMESTAMP WITHOUT TIME ZONE DEFAULT (now() AT TIME ZONE 'utc'), ip_addr VARCHAR(30) DEFAULT '<UNKNOWN>'))SQL","create table");
